...
import { FilePath, RSSG, slugifyFilePath } from "../util/path"
import path from "path"
import workerpool, { Promise as WorkerPromise } from "workerpool"
import { RssgLogger } from "../util/log"
import { trace } from "../util/trace"
import { BuildCtx, WorkerSerializableBuildCtx } from "../util/ctx"
import { styleText } from "util"

export type RssgMdProcessor = Processor<MDRoot, MDRoot, MDRoot>
export type RssgHtmlProcessor = Processor<undefined, MDRoot, HTMLRoot>

export function createMdProcessor(ctx: BuildCtx): RssgMdProcessor {
  ...
  return (
    unified()
      ...
      ) as unknown as RssgMdProcessor
    //  ^ sadly the typing of `use` is not smart enough to infer the correct type from our plugin list
  )
}

export function createHtmlProcessor(ctx: BuildCtx): RssgHtmlProcessor {
  const transformers = ctx.cfg.plugins.transformers
  return (
    unified()
      // MD AST -> HTML AST
...
async function transpileWorkerScript() {
  ...
  const fp = "./rssg/worker.ts"
  return esbuild.build({
    entryPoints: [fp],
    outfile: path.join(RSSG, cacheFile),
    bundle: true,
    keepNames: true,
    platform: "node",
    format: "esm",
...
export function createFileParser(ctx: BuildCtx, fps: FilePath[]) {
  const { argv, cfg } = ctx
  return async (processor: RssgMdProcessor) => {
...
export function createMarkdownParser(ctx: BuildCtx, mdContent: MarkdownContent[]) {
  return async (processor: RssgHtmlProcessor) => {
...
export async function parseMarkdown(ctx: BuildCtx, fps: FilePath[]): Promise<ProcessedContent[]> {
  const { argv } = ctx
  const perf = new PerfTimer()
  const log = new RssgLogger(argv.verbose)
  ...
  if (concurrency === 1) {
    ...
    const pool = workerpool.pool("./rssg/bootstrap-worker.mjs", {
      minWorkers: "max",
