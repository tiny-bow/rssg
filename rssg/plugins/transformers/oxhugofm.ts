...
export interface Options {
  /** Replace {{ relref }} with rssg wikilinks []() */
  wikilinks: boolean
...
const rssgLatexRegex = new RegExp(/\$\$[\s\S]*?\$\$|\$.*?\$/, "g")
 ...
 * markdown to make it compatible with rssg but the list of changes applied it
 * is not exhaustive.
 * */
export const OxHugoFlavouredMarkdown: RssgTransformerPlugin<Partial<Options>> = (userOpts) => {
  ...
  return {
    ...
    textTransform(_ctx, src) {
      ...
      if (opts.replaceOrgLatex) {
        ...
        src = src.replaceAll(rssgLatexRegex, (value) => {
